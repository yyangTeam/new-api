...
func TestAppendBillingInfoSubscriptionClampsNegativeDeltas(t *testing.T) {
	...
	snap := other.Snapshot()
	require.Equal(t, "subscription", snap["billing_source"])
	require.Equal(t, 42, snap["subscription_id"]) // SubscriptionId is int
	require.Equal(t, int64(100), snap["subscription_pre_consumed"])
	// post_delta is surfaced verbatim (can be negative); only the *derived*
	// consumed/remain are clamped.
	require.Equal(t, int64(-150), snap["subscription_post_delta"])
	require.Equal(t, int64(200), snap["subscription_total"])
	// usedFinal = usedAfterPreConsume + postDelta = 80 - 150 = -70 -> clamped 0.
	require.Equal(t, int64(0), snap["subscription_used"])
	// remain = total - usedFinal = 200 - 0 = 200 (not >0 clamp needed here).
	require.Equal(t, int64(200), snap["subscription_remain"])
	// consumed = pre + delta = 100 - 150 = -50 -> clamped 0 -> omitted (only set when >0).
	_, hasConsumed := snap["subscription_consumed"]
	assert.False(t, hasConsumed, "consumed should be omitted when clamped to 0")
	require.Equal(t, 0, snap["wallet_quota_deducted"])
	require.Equal(t, 9, snap["subscription_plan_id"])
	require.Equal(t, "Pro", snap["subscription_plan_title"])
}
...
func TestAppendBillingInfoSubscriptionNormal(t *testing.T) {
	...
	snap := other.Snapshot()
	// consumed = 100 + 20 = 120
	require.Equal(t, int64(120), snap["subscription_consumed"])
	// usedFinal = 80 + 20 = 100
	require.Equal(t, int64(100), snap["subscription_used"])
	// remain = 200 - 100 = 100
	require.Equal(t, int64(100), snap["subscription_remain"])
}
...
func TestAppendBillingInfoWalletNoSubscriptionFields(t *testing.T) {
	...
	appendBillingInfo(info, other)
	snap := other.Snapshot()
	require.Equal(t, "wallet", snap["billing_source"])
...
func TestAppendBillingInfoNilGuards(t *testing.T) {
	...
	assert.NotPanics(t, func() { appendBillingInfo(nil, model.NewLogOther()) })
...
func TestAppendRequestConversionChain(t *testing.T) {
	...
	snap := other.Snapshot()
	chain, ok := snap["request_conversion"].([]string)
	require.True(t, ok)
...
func TestAppendRequestConversionChainEmpty(t *testing.T) {
	...
	_, has := other.Snapshot()["request_conversion"]
	assert.False(t, has)
...
func TestAppendFinalRequestFormatClaude(t *testing.T) {
	...
	require.Equal(t, true, other.Snapshot()["claude"])
}
...
func TestAppendParamOverrideInfo(t *testing.T) {
	...
	require.Equal(t, []string{"max_tokens=128"}, other.Snapshot()["po"])

	// empty -> omitted
	other2 := model.NewLogOther()
	appendParamOverrideInfo(&relaycommon.RelayInfo{}, other2)
	_, has := other2.Snapshot()["po"]
	assert.False(t, has)
}
...
func TestAppendStreamStatusError(t *testing.T) {
	...
	snap := other.Snapshot()
	si, ok := snap["stream_status"].(map[string]interface{})
	require.True(t, ok)
...
func TestAppendStreamStatusNormalEnd(t *testing.T) {
	...
	appendStreamStatus(info, other)
	snap := other.Snapshot()
	si := snap["stream_status"].(map[string]interface{})
...
func TestAppendStreamStatusSkippedWhenNotStream(t *testing.T) {
	...
	appendStreamStatus(info, other)
	_, has := other.Snapshot()["stream_status"]
	assert.False(t, has)
}
...
func TestAppendRequestPathFromContext(t *testing.T) {
	...
	require.Equal(t, "/v1/chat/completions", other.Snapshot()["request_path"])
}
...
func TestAppendRequestPathFallbackToRelayInfo(t *testing.T) {
	...
	require.Equal(t, "https://up.stream/v1/x", other.Snapshot()["request_path"])
}
...
func TestGenerateMjOtherInfo(t *testing.T) {
	...
	snap := other.Snapshot()
	require.Equal(t, 0.004, snap["model_price"])
	require.Equal(t, 1.0, snap["group_ratio"])
	require.Equal(t, 2.0, snap["user_group_ratio"]) // special ratio
	require.Equal(t, "https://up/mj", snap["request_path"])
}
...
func TestGenerateTextOtherInfoBasic(t *testing.T) {
	...
	snap := other.Snapshot()
	require.Equal(t, 2.5, snap["model_ratio"])
	require.Equal(t, 1.0, snap["group_ratio"])
	require.Equal(t, 3.0, snap["completion_ratio"])
	require.Equal(t, 100, snap["cache_tokens"])
	require.Equal(t, 0.5, snap["cache_ratio"])
	require.Equal(t, 0.0, snap["model_price"])
	require.Equal(t, 1.0, snap["user_group_ratio"])
	require.Equal(t, "high", snap["reasoning_effort"])
	require.Equal(t, true, snap["is_model_mapped"])
	require.Equal(t, "gpt-4-0613", snap["upstream_model_name"])
	require.Equal(t, "/v1/chat/completions", snap["request_path"])
	_, hasFrt := snap["frt"]
	...
	_, hasRE := other2.Snapshot()["reasoning_effort"]
...
func TestGenerateClaudeOtherInfo(t *testing.T) {
	...
	snap := other.Snapshot()
	require.Equal(t, true, snap["claude"])
	require.Equal(t, 5, snap["cache_creation_tokens"])
	require.Equal(t, 0.25, snap["cache_creation_ratio"])
	// 5m/1h variants only emitted when non-zero.
	_, has5m := snap["cache_creation_tokens_5m"]
	assert.False(t, has5m)
	_, has1h := snap["cache_creation_tokens_1h"]
	...
	require.Equal(t, 7, snap2["cache_creation_tokens_5m"])
	require.Equal(t, 0.15, snap2["cache_creation_ratio_5m"])
}
...
func TestGenerateAudioOtherInfo(t *testing.T) {
	...
	snap := other.Snapshot()
	require.Equal(t, true, snap["audio"])
	require.Equal(t, 2.0, snap["audio_ratio"])
	require.Equal(t, 3.0, snap["audio_completion_ratio"])
}
...
func TestInjectTieredBillingInfo(t *testing.T) {
	...
	snap := other.Snapshot()
	require.Equal(t, "tiered_expr", snap["billing_mode"])
	require.Equal(t, "tier_a", snap["matched_tier"])
	// expr_b64 must decode back to the original expression string.
	b64, _ := snap["expr_b64"].(string)
...
func TestInjectTieredBillingInfoNoSnapshot(t *testing.T) {
	...
	assert.NotPanics(t, func() { InjectTieredBillingInfo(model.NewLogOther(), nil, nil) })
}
