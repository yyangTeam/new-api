...
describe('authentication session coordination', () => {
  ...
  test('applyAuthRotation throws when no active session exists', () => {
    useAuthStore.getState().auth.reset('idle')
    assert.throws(
      () =>
        applyAuthRotation({
  ...
  test('isAuthBundle rejects non-objects', () => {
    assert.equal(isAuthBundle(null), false)
    assert.equal(isAuthBundle(undefined), false)
    assert.equal(isAuthBundle('string'), false)
    assert.equal(isAuthBundle(42), false)
  })

  test('isAuthBundle rejects missing required fields', () => {
    assert.equal(isAuthBundle({}), false)
    assert.equal(isAuthBundle({ access_token: 'tok' }), false)
    assert.equal(
      isAuthBundle({
        access_token: 'tok',
        token_type: 'Bearer',
  ...
  test('isAuthBundle rejects invalid user (non-integer id)', () => {
    assert.equal(
      isAuthBundle({
        access_token: 'tok',
        token_type: 'Bearer',
        access_expires_at: 100,
        user: { id: 'not-a-number', username: 'u', role: 1 },
        session: bundle.session,
      }),
      false
    )
  ...
  test('isAuthBundle rejects user with id <= 0', () => {
    assert.equal(
      isAuthBundle({
        access_token: 'tok',
        token_type: 'Bearer',
        access_expires_at: 100,
        user: { id: 0, username: 'u', role: 1 },
        session: bundle.session,
      }),
      false
    )
  ...
  test('isAuthBundle rejects empty access_token', () => {
    assert.equal(
      isAuthBundle({
        access_token: '',
        token_type: 'Bearer',
  ...
  test('isAuthBundle rejects non-finite access_expires_at', () => {
    assert.equal(
      isAuthBundle({
        access_token: 'tok',
        token_type: 'Bearer',
        access_expires_at: Infinity,
        user: bundle.user,
        session: bundle.session,
      }),
      false
  ...
  test('isAuthBundle rejects session with empty sid', () => {
    assert.equal(
      isAuthBundle({
        access_token: 'tok',
        token_type: 'Bearer',
        access_expires_at: 100,
        user: bundle.user,
        session: { ...bundle.session, sid: '' },
      }),
      false
    )
  })

  test('a second session mismatch (no retry) returns out_of_sync', async () => {
    const runtime: AuthRefreshRuntime = {
      request: async () => ({
        status: 409,
        data: { code: 'AUTH_SESSION_MISMATCH' },
      }),
      getExpectedSID: () => 'session-x',
      parseBundle: () => null,
      acceptBundle: () => undefined,
      clear: () => undefined,
      markTransient: () => undefined,
    ...
    const outcome = await createRefreshRunner(runtime)()
    assert.equal(outcome.kind, 'out_of_sync')
    assert.equal(
  ...
  test('isCurrent check before request returns superseded', async () => {
    let cleared = false
    const runtime: AuthRefreshRuntime = {
      request: async () => ({ status: 200, data: { success: true, data: bundle } }),
      getExpectedSID: () => 'session-x',
      parseBundle: (value) => (isAuthBundle(value) ? value : null),
      acceptBundle: () => undefined,
      clear: () => { cleared = true },
  ...
  test('response with status 0 (network failure) marks transient', async () => {
    let transientCount = 0
    const runtime: AuthRefreshRuntime = {
      request: async () => ({ status: 0, error: new Error('Network failure') }),
      getExpectedSID: () => 'session-x',
      parseBundle: () => null,
      acceptBundle: () => undefined,
      clear: () => undefined,
  ...
  test('unknown 4xx response clears and returns out_of_sync with code', async () => {
    let cleared = false
    const runtime: AuthRefreshRuntime = {
      request: async () => ({
        status: 403,
    ...
    const outcome = await createRefreshRunner(runtime)()
    assert.equal(outcome.kind, 'out_of_sync')
    assert.equal((outcome as { code?: string }).code, 'FORBIDDEN')
    assert.equal(cleared, true)
  })

  test('bootstrap with stale session triggers refresh', async () => {
    useAuthStore.getState().auth.setBundle({
      ...bundle,
      access_expires_at: Math.floor(Date.now() / 1000) - 100,
    })
    useAuthStore.getState().auth.setBootstrapState('complete')
    // The bootstrap should see the stale token and try to refresh
    const outcome = await bootstrapAuthentication()
    // Since there's no real server, this will complete in some transient state
    // The key is that it doesn't short-circuit to 'anonymous'
    assert.notEqual(outcome.kind, 'anonymous')
  })
